...
def bytestring_to_int(bs):
	try:
		return int('0x'+''.join([hex(ord(i))[2:] for i in bs]),16)
	except TypeError:
		return int('0x'+''.join([hex(i)[2:] for i in bs]),16)
...
class BillerChannel(object):
    ...
    def __init__(self,number, value, currency):
        self._channel = number
        self._value = value
        self._currency = currency

    def __str__(self):
        return '{:.02f} {} {}'.format(self._value, self._currency,self._channel)
...
class Biller(object):
    ...
    (EVT_RESET, EVT_READ, EVT_voucher, EVT_REJECTING, EVT_REJECTED,
    ...
    def __init__(self, port):
        """
            opens the provided serial port name, sets the crc parameters and initiation the communication by syncronizing it,
            and finally loading the settings from the Cash-Reader.
        """
    ...
    def _send(self, command, data=b''):
        """ Send a request.
            ...
            takes the command provided as a byte (aka integer between 0 and 255), and perse it as a command occurding to the 
            protocol in use, the pursing process starts with the sequance then adding the length of data, the command, and the data
            finally we add the crc of all the thing previously added, and then we push stx (aka 0x07) to the beginning and replacing it in the data with a 0x070x07.
            then the data is ready for transmission.
        """
    ...
    def _recv(self):
        """ Receive the response of a request.
            ...
            receives data from serial port write after transmitting a command request, 
            the data is processed based on teh protocol used, then the resulting frame is crc checked and error checked then returned
        """
    ...
    def _transmit(self, command, data=b''):
        """ Send a request.
            ...
            transmits the data command, receives result, and modify sequence number.
        """
    ...
    def _load_settings(self):
        ...
        for ch in range(n_ch):
            off = 12 + ch
            value = ord(s[off]) * multiplier

            off = 16 + n_ch * 2 + ch * 3
            currency = s[off:off + 3].decode('utf8')

            self._channels.append(BillerChannel(ch,value, currency))
    ...
    def channels_set(self, channels):
        """ Set channels.
            ...
            enables the required channels (currencies), and disables the rest
        """
    ...
    def poll(self):
        ...
        while i < len(r):
            ...
            if code in (ssp.EVT_READ,
                ...
                channel = self._channels[ord(r[i + 1]) - 1] if ord(r[i + 1]) else None
                i += 2
            else:
                channel = None
                i += 1
