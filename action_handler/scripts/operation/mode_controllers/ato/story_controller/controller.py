...
from scripts.act_manager import act_manager 

def length_of_act_queue():
    return len(act_manager.act_queue)

def running_action():
    return act_manager.running_action
...
story_current_count = 0
act_index= 0
def run(running_address):
    global stories, pause_list, parent_obj, story_current_count, is_running_thread_flag, act_index, global_running_address
    is_running_thread_flag = True
    while not rospy.is_shutdown():
        if is_running_thread_flag == False or running_address != global_running_address:
            break
        elif parent_obj['status'] == 'halt':
            time.sleep(1)
            continue
            ...
            if act_index == len(stories['stories'][stories['default_story']]['acts']):
                ...
                if stories['stories'][stories['default_story']]['count'] > 0:
                    print('increasing story count ' + str(story_current_count), 'current default story max cound' + str(stories['stories'][stories['default_story']]['count'])  )
...
def get_running_state():
    return str(is_running_thread_flag)

#api
is_running_thread_flag = False
global_running_address = 0
def start():
    global is_running_thread_flag, global_running_address
    if is_running_thread_flag == False:
        global_running_address +=1
        thread = threading.Thread(target=run, args=(global_running_address,))
        thread.start()
        
def stop_story_controller():
    global is_running_thread_flag
    is_running_thread_flag = False
