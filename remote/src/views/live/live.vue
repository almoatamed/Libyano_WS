    ...
    <div v-if="mode=='mnl'">
      ...
      >
        ...
        <v-btn
          :style="
            !angled_goal
              ? 'background:rgb(255,255,255);color:rgb(255,199,74)'
              : 'background:rgb(255,199,74);color:white'
          "
          class="ml-2"
          @click="
            angled_goal = !angled_goal;
            wait_for_goal(angled_goal, 'angled');
          "
        >
        ...
        <v-btn
          ...
          @click="
            ...
            wait_for_goal(unangled_goal, 'unangled');
          "
        >
        ...
        <v-col>
          <v-row>
            ...
            <v-btn
              ...
              @mousedown="
                move_head = true;
                map_scroll ? moveup() : movehead(0, -5);
              "
          ...
          <v-row>
            <v-btn
              ...
              @mousedown="
                move_head = true;
                map_scroll ? moveleft() : movehead(-10, 0);
              "
              @mouseup="move_head = false"
              @touchstart="
                move_head = true;
            ...
            <v-btn
              ...
              @mousedown="
                move_head = true;
                map_scroll ? moveright() : movehead(10, 0);
              "
              @mouseup="move_head = false"
              @touchstart="
                move_head = true;
          ...
          <v-row>
            ...
            <v-btn
              ...
              @mousedown="
                move_head = true;
                map_scroll ? movedown() : movehead(0, 5);
              "
              @mouseup="move_head = false"
              @touchstart="
                move_head = true;
            ...
            <v-col class="pa-0">
              ...
              <v-row>
                <v-col>
                  ...
                  <v-btn
                    text
                    @click="speak_file == '' ? playtemp() : playfile()"
                  >
              ...
              <v-col md="6">
                ...
                >
                  <v-list-item-content class="pa-0">
                    <v-btn
                      text
                      @click="led_return_default = !led_return_default"
                    >
                      return to default
                    </v-btn>
                  </v-list-item-content>
          ...
          <v-card-text>
            ...
            <v-row>
              <v-col md="6">
                ...
                <v-list-item
                  :style="
                    emoji_return_default == true ? 'background:orange' : ''
                  "
                >
                  <v-list-item-content class="pa-0">
                    <v-btn
      ...
      >
        <v-card class="text-center mb-0" width="300">
          <v-card-text>
            ...
            <v-btn
              small
              @click="angled_point = true"
              :color="angled_point ? 'orange' : 'primary'"
              class="mr-2"
              >Angled</v-btn
            >
            <v-btn
              small
              @click="angled_point = false"
              :color="!angled_point ? 'orange' : 'primary'"
              >Unangled</v-btn
            >
            <v-divider class="my-4 secondary" />
            ...
            >
              ...
              >
                <v-list-item-content
                  :style="
                    selected_point.name == point.name ? 'background:orange' : ''
                  "
                  @click="
                    selected_point.name != point.name
            ...
            label="Time"
          /> -->
            <v-btn dark color="primary" @click="go_to_point"> Go </v-btn>
          </v-card-text>
        </v-card>
      </v-menu>
      ...
      >
        <v-card class="text-center mb-0" width="300">
          <v-card-text>
            <strong class="mb-3 d-inline-block font-weight-black"
              >Head Motions</strong
            >
            ...
            <v-col cols="12" md="6" lg="12">
              ...
              >
                ...
                <v-card
                  class="py-2 px-4"
                  :color="
                    !map_scroll ? 'rgba(0, 0, 0, .3)' : 'rgba(255, 175, 0, .3)'
                  "
                  dark
                  flat
...
function translatepose(x, y, q) {
  ...
  var xp =
    map.info.width - (x - map.info.origin.position.x) / map.info.resolution;
...
function drawgoal() {
  if (point.draw && goal_monitor) {
    ...
    if (
      !(
        point.robot_x - goal_monitor["pose"]["x"] < 0.15 &&
        point.robot_y - goal_monitor["pose"]["y"] < 0.15
      )
    ) {
...
export default {
  ...
  methods: {
    ...
    set_defaults() {
      ...
      this.interface_set();
      this.$store.dispatch("Ros/take_action", "interactive/head_move_home", {
        root: true,
      });
      this.$store.dispatch(
        "Ros/take_action",
        "interactive/set_eyes/blink/0/1",
        { root: true }
      );
      this.$store.dispatch(
        "Ros/take_action",
        "interactive/set_ring_flow/fadeoutfadein/0/1",
        { root: true }
      );
    ...
    angled_goal_mouseup_handler(e) {
      if (point.drop && map.info) {
        ...
        point.abs_angle = Math.atan(
          Math.abs((e.layerY - point.layerY) / (e.layerX - point.layerX))
        );
        ...
        this.$store
          .dispatch(
            "Ros/take_action",
            `navigation/angled_goal/${point.robot_x}&${point.robot_y}&${0.0}&${
              qs.x
            }&${qs.y}&${qs.z}&${qs.w}`
          )
          .then((res) => {
    ...
    go_home() {
      this.$store
        .dispatch("Ros/take_action", `navigation/go_home`)
        .then(() => {
          this.clear_goal();
        })
        .catch(() => {});
    ...
    toggle_pause(val) {
      this.$store
        .dispatch("Ros/take_action", `act/toggle_pause/${val}`, { root: true })
        .then((res) => {
          console.log(res);
        });
    },
    stop_and_clear() {
      this.$store
        .dispatch("Ros/take_action", "act/stop_and_clear", { root: true })
        .then((res) => {
          console.log(res);
        });
    },
    ...
    move(direction) {
      if (direction == "w" && this.navigation_manual) {
        this.$store.dispatch("Ros/take_action", "navigation/move/0.23/0", {
          root: true,
        });
      } else if (direction == "a" && this.navigation_manual) {
        this.$store.dispatch("Ros/take_action", "navigation/move/0/0.7", {
          root: true,
        });
        ...
        this.$store.dispatch("Ros/take_action", "navigation/move/0/0", {
          root: true,
        });
      }
    },
    ...
    fetch_head_motions_names() {
      this.$store
        .dispatch("Ros/take_action", "interactive/head_get_motions_names", {
          root: true,
        })
        .then((res) => {
    ...
    set_head_motion() {
      if (this.head_motion != "") {
        this.$store.dispatch(
          "Ros/take_action",
          `interactive/head_play_motions_by_name/${this.head_motion}`
        );
      }
    },
  },
  ...
  mounted() {
    document.addEventListener(
      ...
      (event) => {
        ...
        if ((name == "w" || name == "ArrowUp") && this.navigation_manual) {
          ...
          (name == "a" || name == "ArrowLeft") &&
          this.navigation_manual
        ) {
          this.$store.dispatch("Ros/take_action", "navigation/move/0/0.7", {
            root: true,
          });
        } else if (
          (name == "s" || name == "ArrowDown") &&
          this.navigation_manual
        ) {
          this.$store.dispatch("Ros/take_action", "navigation/move/-0.23/0", {
            root: true,
    ...
    this.$store
      .dispatch("Ros/take_action", "navigation/fetch_points", { root: true })
      .then((res) => {
        if (res == "") {
          console.log("love");
        } else {
