<template>
  <v-container fluid id='root' fill-height class="pa-0 ma-0" >
    ...
    >
        <v-card-text>
            <v-row class="pa-2">
                <v-switch label="Manual" color="purple accent-4" :disabled='control_loading' :loading="control_loading" @change="manual($event,'love')"  v-model="controllers" dark ></v-switch>
            </v-row>
        </v-card-text>
        <v-card-text v-if='controllers'>
            <v-row class="pa-2">
                <v-btn text color="purple accent-4"  @click='go_home'>go home</v-btn>
                <v-switch label="Angled Goal" color="purple accent-4" :disabled='control_loading' :loading="control_loading" @change="wait_for_goal($event,'angled')"  v-model="angled_goal" dark ></v-switch>
                <v-switch label="Unangled Goal" color="purple accent-4" :disabled='control_loading' :loading="control_loading" @change="wait_for_goal($event,'unangled')"  v-model="unangled_goal" dark ></v-switch>
                <v-btn text color="purple accent-4"   @click='clear_goal'>clear</v-btn>
            </v-row>
            <v-row class="pa-2">
                <v-spacer></v-spacer>
            ...
            <v-row class="pa-2">
                <v-btn color="purple accent-4" 
                 @mousedown="continue_zoomin = true; zoomin($event)" 
                 @mouseup="continue_zoomin = false;" 
                 @touchstart="continue_zoomin = true; zoomin($event.touches[0])" 
                ...
                fab small 
                @mousedown="continue_zoomout = true; zoomout($event)" 
                @mouseup="continue_zoomout = false;" 
                @touchstart="continue_zoomout = true; zoomout($event.touches[0])" 
    ...
    methods:{
        ...
        angled_goal_mouseup_handler(e){
            if(point.drop && map.info){
                ...
                this.$store.dispatch('Ros/take_action', `navigation/angled_goal/${point.robot_x}&${point.robot_y}&${0.0}&${qs.x}&${qs.y}&${qs.z}&${qs.w}`).then((res)=>{
                    console.log(res)
                    point.draw = true
                })
        ...
        unangled_goal_mouseup_handler(){
            if(point.drop && map.info){
                ...
                point.robot_x = (map.info.width - point.x)*map.info.resolution + map.info.origin.position.x
                point.robot_y = (point.y)*map.info.resolution + map.info.origin.position.y
                this.$store.dispatch('Ros/take_action', `navigation/unangled_goal/${point.robot_x}&${point.robot_y}`).then((res)=>{
                    console.log(res)
                    point.draw = true
                })
            }
