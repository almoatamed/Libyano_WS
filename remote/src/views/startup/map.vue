<template>
  <v-container fluid id='root' fill-height class="pa-0 ma-0" >
    ...
    >
        <v-card-text >
            <v-row class="pa-2" v-if="arrow_goal_flag || point_goal_flag">
                <v-btn text color="purple accent-4" v-if="arrow_goal_flag" :disabled='goal_flag' @click='wait_for_angled_goal'>Arrow Goal</v-btn>
                <v-btn text color="purple accent-4" v-if="point_goal_flag" :disabled='goal_flag' @click='wait_for_unangled_goal'>Point Goal</v-btn>
            ...
            <v-row class="pa-2">
                <v-spacer></v-spacer>
                <v-switch label="arrow control" color="purple accent-4" v-if="cmd_vel_flag"  v-model="cmd_vel" dark ></v-switch>
                <v-spacer></v-spacer>
            </v-row>
            <v-row class="pa-2">
            ...
            <v-row class="pa-2">
                <v-spacer></v-spacer>
                <v-btn color="purple accent-4" @mousedown="continue_movement_up = true; up($event)" @mouseup="continue_movement_up = false;cmd_vel?stop():null" fab small icon>
                    <v-icon>
                        mdi-chevron-up
                    </v-icon>
                </v-btn>
            ...
            <v-row class="pa-2">
                <v-btn color="purple accent-4" @mousedown="continue_movement_left = true; left($event)" @mouseup="continue_movement_left = false;cmd_vel?stop():null"  fab small icon>
                    <v-icon>
                        mdi-chevron-left
                    </v-icon>
                </v-btn>
                ...
                <v-spacer></v-spacer>
                <v-btn color="purple accent-4" @mousedown="continue_movement_right = true; right($event)" @mouseup="continue_movement_right = false;cmd_vel?stop():null" fab small icon>
                    <v-icon>
                        mdi-chevron-right
                    </v-icon>
                </v-btn>
            ...
            <v-row class="pa-2">
                <v-spacer></v-spacer>
                <v-btn color="purple accent-4" @mousedown="continue_movement_down = true; down($event)" @mouseup="continue_movement_down = false;cmd_vel?stop():null" fab small icon>
                    <v-icon>
                        mdi-chevron-down
                    </v-icon>
                </v-btn>
...
export default {
    data(){
        return{
            ...
            cmd_vel_flag: null,
            point_goal_flag: null, 
            arrow_goal_flag: null,

            navigation_mode_interval_holder:null
    ...
    mounted(){
        ...
        this.$store.dispatch('Loading/clear')
        ...
        this.navigation_mode_interval_holder = setInterval(()=>{
          this.$store.dispatch('Ros/take_action', 'navigation/get_mode', {root:true}).then(res=>{
            if(res == 'manual'){
                this.$store.dispatch('Ros/take_action', 'startup/get_manual_controls', {root:true}).then(res=>{
                    this.controllers = true
                    console.log(res.split('|'))
                    this.manual_control_flags = res.split('|')
                    if(!this.manual_control_flags.includes('cmd_vel')){
                        this.cmd_vel = false
                    }
                    this.arrow_goal_flag = this.manual_control_flags.includes('arrow_goal') ? true : false
                    this.cmd_vel_flag = this.manual_control_flags.includes('cmd_vel') ? true : false
                    this.point_goal_flag = this.manual_control_flags.includes('point_goal') ? true : false
                })
            }else{
              this.controllers = false
              this.manual_control_flags = []
            }
          })
        },400)
